     ...
     for (let i = 0; i < data.length; i++) {
          for (let j = 0; j < data[i].songs.length; j++) {
               if(x === data[i].songs[j] && y === data[i].singer){
                    indSongsDis += '<div id = "ind-songs-bar-' + data[i].name + '" class="songs-bar"  onclick = "indOpenModal(' + "'" + data[i].duu[j] + "', '" + data[i].name + "'"+')">'
     ...
     for (let i = 0; i < data.length; i++) {
          for (let j = 0; j < data[i].songs.length; j++) {
               if (nameSong === data[i].songs[j] && nameArtist === data[i].name) {
                    ...
                    next = true;
                    indOpenModal(data[i].duu[j+1], data[i].name);
                    break;    
               }
          }
     }
...
function indReplayTrack(){
     for (let i = 0; i < data.length; i++) {
          for (let j = 0; j < data[i].songs.length; j++) {
               if (nameSong === data[i].songs[j]) {
                    next = true;
                    indOpenModal(data[i].duu[j], data[i].name,);
                    break;
               }
          }
     }
